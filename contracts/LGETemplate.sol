...
pragma solidity =0.6.8;
contract LGE_TEMPLATE_CRO is ERC20, Ownable {
    ...
    address public constant tokenTeam = address(0x);
    address public constant marketingTeam = address(0x);

    address public constant REWARD_TREASURY = address(0x);
    uint256 private constant TEAM_PERCENTAGE = 0.05 ether;
    uint256 private constant OTHER_PERCENTAGE = 0.05 ether;
    uint256 private constant BURN_FEE = 0.001 ether;
    ...
    modifier onlyPair() {
        require(
            msg.sender == tokenPair,
            "Empire::onlyPair: Insufficient Privileges"
        );
        _;
    }

    constructor() public ERC20("TOKEN", "TOKEN") Ownable() {
    }
    ...
    function beginLGE(IEmpireFactory _factory) external onlyOwner() {
        ...
        tokenPair = _factory.createPair(WCRO, address(this), pairType, 0);
    }
    ...
    function complete() external onlyOwner() {
        ...
        payable(tokenTeam).sendValue(teamAllocation);
        payable(marketingTeam).sendValue(otherAllocation);

        _totalRaised = address(this).balance;
        IWCRO(WCRO).deposit{value: _totalRaised}();
        _mint(tokenPair, 5000 * 1 ether); 
        IERC20(WCRO).safeTransfer(tokenPair, _totalRaised);
        IEmpirePair(tokenPair).mint(address(this));
        _mint(address(this), 1000 * 1 ether); 
        _mint(tokenTeam, 1000 * 1 ether); 
        _mint(REWARD_TREASURY, 3000 * 1 ether);
    }

    function claim() external {
        require(
            contributions[msg.sender] > 0,
            "TOKEN::claim: No contribution detected!"
        );
    ...
    function sweep(uint256 amount, bytes calldata data) external onlyOwner() {
        IEmpirePair(tokenPair).sweep(amount, data);
    }

    function empireSweepCall(uint256 amount, bytes calldata) external onlyPair() {
        IERC20(WCRO).safeTransfer(owner(), amount);
    }

    function unsweep(uint256 amount) external onlyOwner() {
        IERC20(WCRO).approve(tokenPair, amount);
        IEmpirePair(tokenPair).unsweep(amount);
    }
    ...
    function transferFrom(
        ...
        uint256 burned;
        if (from != tokenPair && to != tokenPair) {
    ...
    function transfer(address to, uint256 amount)
        ...
        returns (bool)
    {
        if (msg.sender != tokenPair && to != tokenPair) {
